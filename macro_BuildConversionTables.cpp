...
{
  const double GasPressure=869; //mbar //In the LNL experiment: 873 mbar, 869 mbar, 579 mbar, 253 mbar, 243 mbar, 355 mbar, 690 mbar
  const double GasTemperature=869; //K
  ...
  const double BeamEnergy=69; //MeV //In the LNL experiment: 69 MeV, 59 MeV, 48 MeV
  const int Zbeam=10;
  const int Abeam=20;
  const int Ztarget=2;
  const int Atarget=4;
  ...
  if(KinematicSolution.compare("Hi")==0 || KinematicSolution.compare("Low")==0) {
    FileName.assign(Form("output/REXpp_ConversionParameters_%s_%s_Z%02d_A%02d_%s%.2fum_%.0fmbar%.2fK_%.0fMeV_%s.dat",EntranceChannel.c_str(),ExitChannel.c_str(),Zparticle, Aparticle, WindowMaterial.c_str(),WindowThickness,GasPressure,GasTemperature,BeamEnergy,KinematicSolution.c_str()));
    FileBufferName.assign(Form("/mnt/nas0/dellaquila/201904_Mg24/output/REX++/REXpp_ConversionBuffer_%s_%s_Z%02d_A%02d_%s%.2fum_%.0fmbar%.2fK_%.0fMeV_%s.root",EntranceChannel.c_str(),ExitChannel.c_str(),Zparticle, Aparticle, WindowMaterial.c_str(),WindowThickness,GasPressure,GasTemperature,BeamEnergy,KinematicSolution.c_str()));
  } else {
    FileName.assign(Form("output/REXpp_ConversionParameters_%s_%s_Z%02d_A%02d_%s%.2fum_%.0fmbar%.2fK_%.0fMeV.dat",EntranceChannel.c_str(),ExitChannel.c_str(),Zparticle, Aparticle, WindowMaterial.c_str(),WindowThickness,GasPressure,GasTemperature,BeamEnergy));
    FileBufferName.assign(Form("/mnt/nas0/dellaquila/201904_Mg24/output/REX++/REXpp_ConversionBuffer_%s_%s_Z%02d_A%02d_%s%.2fum_%.0fmbar%.2fK_%.0fMeV.root",EntranceChannel.c_str(),ExitChannel.c_str(),Zparticle, Aparticle, WindowMaterial.c_str(),WindowThickness,GasPressure,GasTemperature,BeamEnergy));
  }
      ...
      for(int i=0; i<TheDetectors->fmulti; i++) {
        if(TheDetectors->fZ[i]==Zparticle && TheDetectors->fA[i]==Aparticle) {
          if(TheDetectors->fnumdet[i]>=0 && TheDetectors->fnumstripf[i]>=0 && TheDetectors->fnumstripb[i]>=0) {
            ...
            (HistoKinEnergyOrigin[TheDetectors->fEx])[TheDetectors->fnumdet[i]][TheDetectors->fnumstripf[i]][TheDetectors->fnumstripb[i]]->Fill(TheDetectors->fKinEnergyOrigin[i]);
          }
        }
      }
      //
